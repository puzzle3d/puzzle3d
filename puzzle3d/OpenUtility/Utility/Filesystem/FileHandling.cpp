...
#ifdef WIN32
	dirList.hFile=CreateFile(file,GENERIC_READ,0,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
	size_t size=::GetFileSize(dirList.hFile,NULL);
	CloseHandle(dirList.hFile);
	return(size);
#else
	struct stat st;
	stat(file,&st);
