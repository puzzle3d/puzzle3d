...
//#define M(row,col) matrix[(col<<2)+row]
//#define MR(row,col) rotmatrix[(col<<2)+row]
#define M(col,row) matrix[(col<<2)+row]
#define MR(col,row) rotmatrix[(col<<2)+row]
	MR(0,0)=M(0,0)=dview.x;
	MR(0,1)=M(0,1)=dcros.x;
	MR(0,2)=M(0,2)=dnorm.x;
	MR(3,0)=0.0;
	M(3,0)=pos.x;
	MR(1,0)=M(1,0)=dview.y;
	MR(1,1)=M(1,1)=dcros.y;
	MR(1,2)=M(1,2)=dnorm.y;
	MR(3,1)=0.0;
	M(3,1)=pos.y;
	MR(2,0)=M(2,0)=dview.z;
	MR(2,1)=M(2,1)=dcros.z;
	MR(2,2)=M(2,2)=dnorm.z;
	MR(3,2)=0.0;
	M(3,2)=pos.z;
	MR(0,3)=M(0,3)=0.0;
	MR(1,3)=M(1,3)=0.0;
...
void Matrix44Mul44f(float *product,const float *a,const float *b)
{
#define A(row,col) a[(col<<2)+row]
#define B(row,col) b[(col<<2)+row]
#define P(row,col) product[(col<<2)+row]
...
void Matrix34Mul31f(XYZf *product,const float *a,const XYZf *b)
{
#define A(row,col) a[(col<<2)+row]
#define P(row,col) product[(col<<2)+row]
...
void Matrix44Mul31f(XYZf *product,const float *a,const XYZf *b)
{
#define A(row,col) a[(col<<2)+row]
#define P(row,col) product[(col<<2)+row]
...
void Invert44f(float *invert, const float *mat){
	float Transposee[16];
	float Det;
#define M(col,row) mat[(col<<2)+row]
#define I(col,row) invert[(col<<2)+row]
#define T(col,row) Transposee[(col<<2)+row]
	
	// Calcul du determinant de mat
	Det =	M(0,0) * (M(1,1)*M(2,2)-M(2,1)*M(1,2))
			- M(0,1) * (M(1,0)*M(2,2)-M(2,0)*M(1,2))
			+ M(0,2) * (M(1,0)*M(2,1)-M(2,0)*M(1,1));

	// Calcul de la matrice de cofacteurs de mat
	T(0,0) = +( M(1,1)*M(2,2)-M(2,1)*M(1,2) );
	T(1,0) = -( M(0,1)*M(2,2)-M(2,1)*M(0,2) );
	T(2,0) = +( M(0,1)*M(1,2)-M(1,1)*M(0,2) );
	T(0,1) = -( M(1,0)*M(2,2)-M(2,0)*M(1,2) );
	T(1,1) = +( M(0,0)*M(2,2)-M(2,0)*M(0,2) );
	T(2,1) = -( M(0,0)*M(1,2)-M(1,0)*M(0,2) );
	T(0,2) = +( M(1,0)*M(2,1)-M(2,0)*M(1,1) );
	T(1,2) = -( M(0,0)*M(2,1)-M(2,0)*M(0,1) ),
	...
	// On divise la matrice des cofacteurs par le determinant
	for(int i=0;i<16;i++){
		Transposee[i]/=Det;
	}

	// On transpose pour obtenir la matrice inverse
	I(0,0) = T(0,0);
	I(0,1) = T(1,0);
	I(0,2) = T(2,0);
	I(0,3) = T(3,0);
